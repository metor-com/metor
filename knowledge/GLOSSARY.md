...
| **Harness / Runtime** | "Runtime" | `harness: claude-stream \| codex \| gemini` | — | The agent runner that drives the model (Claude Code, Codex, Gemini CLI) – runs as a process inside the computer; registry in `metor-harness.mjs` (ADR-0011), each bot additionally carries `model` |
