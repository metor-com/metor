...
uses SSH2, restricted initially to ED25519 host keys and root password authentication.
It discovers the fingerprint without authenticating, asks the user to compare it with
the provider console, and pins that fingerprint before sending a password. Rebuilding
a VPS therefore requires verification again; no global SSH known_hosts entry is changed.
Passwords are not logged or persisted, and are cleared after authentication. The
...
SSH keys, non-root sudo, custom proxies and ongoing remote RAM management are follow-ups.
