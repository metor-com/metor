...
| **Streaming** | `item/agentMessage/delta` delivers text deltas (→ partial.json); `item/completed` with item.type `agentMessage` (field `phase`: `commentary` = interim text, `final_answer`), `mcpToolCall` {server,tool,arguments,result,durationMs}, `commandExecution`, `reasoning` (its summary/content – shown live as the thinking, never kept), `userMessage` (echo). The reasoning also streams as `item/reasoning/summaryTextDelta`, `item/reasoning/summaryPartAdded` and `item/reasoning/textDelta` (names read from the binary, 2026-09-08) | S19/S20 |
