...
export function parseServer(out) {
  ...
  return { os, cpus, ram, disk, memory, addresses: (/^METOR_ADDRESSES:(.*)$/m.exec(out)?.[1] ?? '').trim().split(/\s+/).filter(isIP), resume: /^METOR_RESUME:(\w+)$/m.exec(out)?.[1] ?? null };
}
    ...
    conn.exec('bash -s', (error, stream) => {
      ...
      stream.on('close', code => {
        if (code !== 0) {
          ...
          const phase = [...output.matchAll(/^METOR_PHASE:(\w+)$/gm)].at(-1)?.[1];
...
export function installationError(phase) {
  return ({
    docker: 'Docker installation failed. Check outbound HTTPS access and package-manager availability on the server, then retry.',
    image: 'The metor image download failed. Check outbound access to ghcr.io and free disk space, then retry. Existing data is retained.',
    files: 'Could not prepare the installation files. Check free disk space and directory permissions, then retry.',
    start: 'The containers could not start. Check Docker and conflicts on ports 80, 443 and 6010, then retry.',
    gateway: 'The Space did not become ready. Check Docker container health and available RAM, then retry.',
    ready: 'metor is installed, but device pairing could not be prepared. Reconnect to retry without reinstalling.',
  })[phase] || 'The server command failed. Check Docker, network access and available disk space, then retry.';
}
...
export function signInError(error, mismatch = false, method = 'password') {
  if (mismatch) return 'The server identity changed. Verify its fingerprint again before signing in.';
  if (error?.level === 'client-authentication' && method === 'key') return 'The server rejected this SSH key for root. Select the private key matching the public key installed on the server.';
...
async function inspectSession(input, inspectConnected) {
  ...
  let auth;
  if (method === 'key') {
    if (!Buffer.isBuffer(input.privateKey) || input.privateKey.length > 65536) throw Error('Choose a private SSH key file.');
    if (input.passphrase && (typeof input.passphrase !== 'string' || input.passphrase.length > 4096)) throw Error('Invalid key passphrase.');
    const parsed = utils.parseKey(input.privateKey, input.passphrase || undefined);
    if (parsed instanceof Error || Array.isArray(parsed) || !parsed.isPrivateKey()) {
      input.privateKey.fill(0); input.passphrase = '';
      throw Error('Could not unlock a private SSH key. Choose a private key, not a .pub file, and check its passphrase.');
    }
    auth = { privateKey: input.privateKey, passphrase: input.passphrase || undefined };
  } else {
  ...
  try {
    await new Promise((resolve, reject) => {
      conn.on('error', error => reject(Error(signInError(error, mismatch, method))));
      ...
      conn.connect({ ...address, username: 'root', ...auth, readyTimeout: 20000,
    ...
    if (Buffer.isBuffer(input.privateKey)) input.privateKey.fill(0);
    auth.password = undefined; auth.passphrase = undefined; auth.privateKey = undefined;
    if (conn.config) { conn.config.password = undefined; conn.config.privateKey = undefined; conn.config.passphrase = undefined; }
  }
}
export async function install(session, installer, version, progress = () => {}) {
  ...
  if (info.resume === 'ready') {
    progress('Checking the existing Space before reconnecting…');
    const out = await run(conn, `set -eu\nexec 9>/run/metor-desktop-install.lock\nflock -n 9 || { echo "METOR_ERROR:Another setup is running. Wait before reconnecting."; exit 1; }\ncd /opt/metor\ncode=$(docker compose exec -T box curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:6010/bots/api/harnesses)\ncase "$code" in 200|401) ;; *) echo 'METOR_ERROR:The existing Space is not ready. Check its container logs before retrying.'; exit 1 ;; esac\ndocker compose exec -T box metor auth link\n`);
    return setupLink(out, domain);
  }
