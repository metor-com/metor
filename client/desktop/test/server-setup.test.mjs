...
async function server(t, command, changePassword = false) {
  let credentials = 0;
  const peers = new Set();
  const s = new Server({ hostKeys: [key] }, c => {
    peers.add(c); c.on('error', () => {}); c.on('close', () => peers.delete(c));
    c.on('authentication', ctx => { if (ctx.method === 'password') { credentials++; if (changePassword) return ctx.requestChange('Change your password', () => ctx.reject()); } ctx.method === 'password' && ctx.password === 'test-secret' ? ctx.accept() : ctx.reject(); });
    c.on('ready', () => c.on('session', accept => accept().on('exec', (accept) => {
      const stream = accept(); let script = '';
      stream.on('data', b => script += b.toString());
      stream.on('end', () => { const reply = command?.(script); stream.write(typeof reply === 'string' ? reply : 'METOR_SERVER:ubuntu 26.04|2|3900|42000|2816\n'); stream.exit(0); stream.end(); });
    })));
  });
  await new Promise(resolve => s.listen(0, '127.0.0.1', resolve));
  t.after(async () => { for (const c of peers) c.end(); await new Promise(resolve => s.close(resolve)); });
  return { host: '127.0.0.1', port: s.address().port, credentials: () => credentials };
}
...
test('installation pins the release, allocates RAM, and keeps the setup token out of progress', async t => {
  const s = await server(t, script => {
    ...
    assert.ok(script.indexOf('Existing Docker containers') < script.indexOf('mkdir -p /opt/metor'));
