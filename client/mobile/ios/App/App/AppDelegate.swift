...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        UNUserNotificationCenter.current().delegate = self   // taps and foreground presentation (ADR-0017)
        return true
    }
    ...
    // A tap opens the bot the notification is about (put there by the notification service extension)
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        let bot = response.notification.request.content.userInfo["metor_bot"] as? String ?? ""
