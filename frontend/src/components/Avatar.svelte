<script>
  ...
  import { picture } from "../lib/media.js";   // inside the phone app the picture is fetched with the session (media.js)
...
{#if src}
  <img class="shrink-0 rounded-full bg-zinc-200 object-cover" style="width:{size}px;height:{size}px" use:picture={src} alt="" />
{:else}
