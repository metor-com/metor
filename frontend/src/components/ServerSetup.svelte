<script>
  ...
  onDestroy(() => { off?.(); app.server.cancel(); password = ''; });
  ...
  async function inspect() {
    busy = true; error = '';
    try {
      const result = app.server.inspect({ ...identity, domain, password }); password = '';
  ...
  function reset() { app.server.cancel(); identity = null; info = null; verified = false; error = ''; progress = ''; password = ''; }
</script>
<main class="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-7 shadow-sm">
  ...
  {:else if !info && !installing}
    ...
    <form class="mt-4 flex flex-col gap-3" on:submit|preventDefault={inspect}>
      ...
      <p class="text-xs text-zinc-500">Used for this setup only. The password is never saved.</p>
      <button class={primary} disabled={busy || !verified}>{busy ? 'Checking server…' : 'Check server'}</button>
  ...
  {:else if info && !installing}
    ...
    <button type="button" class="mt-4 w-full {primary}" disabled={busy} on:click={install}>Install and connect</button>
  {/if}
